...
class Agent:
    ...
    def __init__(self, actor_learning_rate, critic_learning_rate, input_dims, tau, env, gamma=0.99,
        ...
        self.max_action = env.action_spec[1]  # Upper action bounds
        self.min_action = env.action_spec[0]
    ...
    def update_network_parameters(self, tau=None):
        ...
        for name in critic_1_state_dict:
            critic_1_state_dict[name] = (
                        tau * critic_1_state_dict[name].clone() + (1 - tau) * target_critic_1_state_dict[name].clone())

        for name in critic_2_state_dict:
            critic_2_state_dict[name] = (
                        tau * critic_2_state_dict[name].clone() + (1 - tau) * target_critic_2_state_dict[name].clone())
